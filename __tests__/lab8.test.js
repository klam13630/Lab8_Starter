describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const innerText = await button[0].getProperty('innerText');
    expect(innerText['_remoteObject'].value).toBe('Remove from Cart');
    // TODO - Step 2
    // Query a <product-item> element using puppeteer ( check out page.$() and page.$$() in the docs )
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const innerText = await cart_count[0].getProperty('innerText');
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 0; i < prodItems.length; i++) {
      ...
      const button = await root.$$('button');
      const innerText = await button[0].getProperty('innerText');
      correctText = innerText['_remoteObject'].value == 'Remove from Cart';
    }
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 0; i < prodItems.length; i++) {
      ...
      const button = await root.$$('button');
      const innerText = await button[0].getProperty('innerText');
      correctText = innerText['_remoteObject'].value == 'Add to Cart';
    }
    expect(correctText).toBe(true);
    const cart_count = await page.$$('#cart-count');
    const innerText = await cart_count[0].getProperty('innerText');
